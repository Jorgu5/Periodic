  ...
  $.ajax({
    ...
    success: function(content) {
      ...
      $.each(content, function(i, obj) {
        $(".periodic").append(
          `<div class="periodic__tile" data-id="${obj.id}">
            <div class="periodic__top">
                <span class="periodic__top_element periodic__id">${
                  obj.id
                }</span>
                <span class="periodic__top_element periodic__short" contenteditable="true">${
                  obj.cat
                }</span>
            </div>
      ...
      $(".periodic__tile").click(function() {
        $(this).toggleClass("periodic__tile_active");
      });
    }
  });
});
...
$(document).ready(function() {
  ...
  setTimeout(function() {
    for (var a = 0; a < greenArray.length; a += 1) {
      $(".periodic__grid")
        .find('[data-id="' + greenArray[a] + '"]')
        .addClass("periodic__tile_green");
    }
    for (var b = 0; b < yellowArray.length; b += 1) {
      $(".periodic__grid")
        .find('[data-id="' + yellowArray[b] + '"]')
        .addClass("periodic__tile_yellow");
    }
    for (var c = 0; c < redArray.length; c += 1) {
      $(".periodic__grid")
        .find('[data-id="' + redArray[c] + '"]')
        .addClass("periodic__tile_red");
    }
    for (var d = 0; d < primaryArray.length; d += 1) {
      $(".periodic__grid")
        .find('[data-id="' + primaryArray[d] + '"]')
        .addClass("periodic__tile_primary");
    }
  }, 1000);
});
